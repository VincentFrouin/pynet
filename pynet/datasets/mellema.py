...
"""
Module that provides functions to prepare the IMPAC dataset according
to the experimental setting of the paper 
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6859452.

IMPAC stands for IMaging-PsychiAtry Challenge: predicting autism which 
is a data challenge on Autism Spectrum Disorder detection:
...
@Fetchers.register
def fetch_mellema(datasetdir, mode="train", dtype="all"):
    """ Fetch/prepare the IMPAC dataset for pynet and paper exp setting
    ...
    item: namedtuple
        ...
        'metadata_path'.
    """
    logger.info("Loading impac dataset the C. Mellema way.")
    if not os.path.isdir(datasetdir):
        os.mkdir(datasetdir)
    train_desc_path = os.path.join(datasetdir, "pynet_mellema_train.tsv")
    selected_input_path = os.path.join(
            datasetdir, "pynet_mellema_inputs_selection.npy")
    train_input_path = os.path.join(
        datasetdir, "pynet_mellema_inputs_train.npy")
    train_output_path = os.path.join(
        datasetdir, "pynet_mellema_outputs_train.npy")
    test_desc_path = os.path.join(datasetdir, "pynet_impac_test.tsv")
    test_input_path = os.path.join(
        datasetdir, "pynet_mellema_inputs_test.npy")
    test_output_path = os.path.join(
        datasetdir, "pynet_mellema_outputs_test.npy")
    if not os.path.isfile(train_desc_path):
        ...
        data['participants_sex_asd'] = \
            np.asarray(['{}_{}'.format(d,s) 
                for d,s in zip(data['participants_asd'],
                               data['participants_sex'])])
        ...
        #create balanced test/train sets (disease AND sex)
        from sklearn.model_selection import StratifiedKFold
        _X = data.index.values
        _y = data['participants_sex_asd']
        skf = StratifiedKFold(n_splits=5, shuffle=True)
        skf.get_n_splits(_X, _y)
        _s = list(skf.split(_X, _y))[0]
        sets["train"], sets["test"] = [str(_X[i]) for i in _s[0]], \
                                      [str(_X[i]) for i in _s[1]]
        for tt in ['train', 'test']:
            with open(os.path.join(datasetdir, "{0}.csv".format(tt)), "w") as fp:
                fp.write('\n'.join(sets[tt]))
